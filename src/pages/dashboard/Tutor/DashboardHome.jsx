...
import { 
  Briefcase, Clock, CheckCircle, DollarSign, ArrowUpRight, MoreHorizontal, Bell
} from 'lucide-react';
...
const DashboardHome = () => {
  ...
  const { 
    ...
    queryFn: async () => {
      ...
      const response = await axios.get(`${API_URL}/api/profile/dashboard-stats/${userId}`);
  ...
  const { stats, revenueData, recentApplications, userName } = useMemo(() => {
    if (!dashboardData?.success) {
      return {
        ...
        revenueData: [{ name: 'No Data', amount: 0 }],
        recentApplications: [],
        userName: 'Tutor'
      };
    }
    ...
    return {
      stats: data.stats,
      revenueData: data.revenueData?.length > 0 ? data.revenueData : [{ name: 'No Data', amount: 0 }],
      recentApplications: data.recentApplications || [],
      userName: data.userName || "Tutor"
    };
  }, [dashboardData]);
  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
      ...
      {/* --- WELCOME BANNER --- */}
      <div className="bg-gradient-to-r from-emerald-700 to-teal-600 rounded-2xl p-6 lg:p-10 text-white shadow-xl relative overflow-hidden">
        <div className="relative z-10">
          <div className="flex justify-between items-start">
            <div>
              <h1 className="text-3xl font-bold mb-2">Welcome back, {userName}! 👋</h1>
              <p className="text-emerald-100 text-lg opacity-90 max-w-xl">
                Your dashboard is ready. You currently have <span className="font-bold text-yellow-300">{stats.pendingReview}</span> applications pending review.
              </p>
            </div>
          ...
          <div className="mt-8 flex gap-4">
             <Link to="/tutor-dashboard/browse-tuitions" className="bg-white text-emerald-700 px-6 py-2.5 rounded-lg font-bold shadow-sm hover:shadow-lg hover:bg-gray-50 transition-all flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-all group">
          ...
          <p className="text-xs text-gray-400 mt-4">Waiting for response</p>
        </div>
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-all group">
          ...
          <p className="text-xs text-emerald-600 font-medium mt-4">Currently Teaching</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <table className="w-full">
              <thead>
                <tr className="text-left text-xs font-semibold text-gray-500 uppercase tracking-wider border-b border-gray-100">
                  <th className="pb-3 pl-2">Subject / Title</th>
                  ...
                  recentApplications.map((app) => (
                    <tr key={app._id} className="hover:bg-gray-50/50 transition-colors">
                      <td className="py-4 pl-2">
                        <p className="text-sm font-semibold text-gray-800 line-clamp-1 max-w-[200px]" title={app.tuitionTitle || app.subject}>
                      ...
                      <td className="py-4 text-sm font-medium text-gray-800">
                        ৳ {app.salary ? app.salary : 'Negotiable'}
                      </td>
                      <td className="py-4">
                        <span className={`px-2.5 py-1 rounded-full text-xs font-bold ${getStatusStyle(app.status)}`}>
                          {app.status}
                        </span>
                      </td>
                      <td className="py-4 text-right pr-2">
                        <button className="text-gray-400 hover:text-emerald-600 transition-colors p-2 hover:bg-gray-100 rounded-full">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex flex-col">
          <h3 className="text-lg font-bold text-gray-800 mb-2">Revenue History</h3>
